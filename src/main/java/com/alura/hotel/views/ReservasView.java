...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtFechaEntrada.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				//Activa el evento, después del usuario seleccionar las fechas se debe calcular el valor de la reserv
	...
	protected void calcularValor() {
		...
		fechaEntrada = new java.sql.Date(fechaEntradaUtil.getTime());
		fechaSalida = new java.sql.Date(fechaSalidaUtil.getTime());
		
		 long diferenciaMilisegundos = fechaSalida.getTime() - fechaEntrada.getTime();
		 long diferenciaDays = TimeUnit.MILLISECONDS.toDays(diferenciaMilisegundos) + 1;
		 
		int valorInt = 0;
		try {
		valorInt = Math.toIntExact(diferenciaDays);
		if(fechaEntrada.compareTo(fechaSalida) < 0) {
			 valor =  valorInt * 200;
			txtValor.setText(" $ " + valor);
		} else {
				JOptionPane.showMessageDialog(null, "La fecha Salida debe ser mayor que Fecha Incio");
			}
		}
	...
	private void guardarRserva() {
		...
		if(txtFechaEntrada.getDate().equals(null) || txtFechaSalida.getDate().equals(null) 
		  ...
		  //Date fehcaEntrada = Date.valueOf(fechaEntrada);
		   //Date fechaSalidaDate = Date.valueOf(fechaSalida);
		  
		 
		   var nuevaReserva = new Reserva(fechaEntrada , fechaSalida, valor, 
				   (String)txtFormaPago.getSelectedItem());
		   this.reservaController.guardar(nuevaReserva);
		   
			
		  //String nuevReserva = nuevaReserva.toString();
		  //Integer nuevaReserv = Integer.parseInt(nuevReserva);
		   numReserva = nuevaReserva.getId();
		   JOptionPane.showMessageDialog(this, "Resgistrado con exito! -> Numero de reserva: " + nuevaReserva.getId());
